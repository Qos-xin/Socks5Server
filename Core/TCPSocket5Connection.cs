...
namespace Kingthy.Test.Socks5.Server.Core
{
    ...
    public class TCPSocks5Connection
    {
        ...
        /// TCPSocks5Connection
        /// </summary>
        public TCPSocks5Connection(TCPSocks5Server server, TCPSocket5Client localClient, Socket client)
        {
            this.Server = server;
            this.Client = client;
        ...
        private TCPSocket5Client LocalClient { get; set; }
        /// <summary>
        /// 客户端
        /// </summary>
        ...
        private void CreateProxyBridge()
        {
            ...
            {
                this.Proxy = LocalClient.tcpClient;
                try
                {
                ...
                try
                {
                    ...
                    if (socket.Connected)
                    {
                        ...
                        if (size > 0)
                        {
                            ...
                            //this.Server.WriteLineLog(string.Format("从客户端{0}接收到数据,大小{1} Byte", socket.RemoteEndPoint, size));
                            if (this.Server.IsStarting)
                                socket.BeginReceive(_ClientBuffer, 0, _ClientBuffer.Length, SocketFlags.None, this.OnClientReceive, socket);
                    ...
                    {
                        this.Server.WriteLineLog("接收到数据,但客户端ClientSocket已关闭");
                    }
                }
                catch (Exception ex)
                {
        ...
        private void OnProxyReceive(IAsyncResult result)
        {
            if (this.Server.IsStarting)
            {
                try
                {
                    ...
                    {
                        this.Server.WriteLineLog("接收到数据,但服务端ProxySocket已关闭");
                    }
                }
                catch (Exception ex)
                {
        ...
        private void Close()
        {
            if (this.Client != null)
            {
                this.Server.WriteLineLog(string.Format("代理端已于{0}关闭与本地客户端{1}的连接...", DateTime.Now, this.Client?.RemoteEndPoint));
                this.Client?.Close(3);
                this.Client = null;
            }
            if (this.Proxy != null)
        ...
        public static void DoRequest(TCPSocks5Server server, TCPSocket5Client localClient, Socket client)
        {
            TCPSocks5Connection connection = new TCPSocks5Connection(server, localClient, client);
